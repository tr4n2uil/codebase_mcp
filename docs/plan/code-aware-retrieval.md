...
|------------|----------------|--------|----------------------|
| **Chunking** — structure-aware splits, symbol + path in text | Biggest | **Partial** | **`src/chunker.ts`**: `chunkCodeAware()` uses **tree-sitter** declaration spans (supported langs) **merged** with line-regex, then line windows for long spans. Fallback: `chunkByLines()`. Toggle: `CODEBASE_MCP_CODE_AWARE_CHUNKING` (default on). **Product goal:** prefer **AST-/class-shaped** units over blind windows so middleware and cohesive types are not split into meaningless snippets (**`docs/representation-inputs-plan.md`** §2). **`Rakefile` / `*.rake` as Ruby:** ensure `detectLanguage` (+ **`lang`** maps in **`path-query-filter.ts`**) treat them like **`*.rb`**. See **`src/tree-sitter-definitions.ts`**, `CODEBASE_MCP_DEF_ENGINE`. **`src/indexer.ts`** `embeddingTextForChunk()` prefixes each embedded string with **`path=…`**, optional **`lang=…`**, **`symbol=…`**, **`kind=…`**; **`def=…`** by default (`CODEBASE_MCP_EMBED_DEF_TAG`; opt out with `0`). **Not done:** import-graph chunking, dedicated “header + body” chunks. |
...
| **Cross-encoder rerank (top-K)** | Big | **Partial (opt-in)** | **`src/cross-encoder-rerank.ts`**, wired in **`src/mcp-tools.ts`**. When **`CODEBASE_MCP_CROSS_ENCODER=1`**, after hybrid + **`rerankSearchHits`**, score **(query, passage)** with **`Xenova/bge-reranker-base`** (default; ONNX via Transformers.js) on the top **K** (`CODEBASE_MCP_CROSS_ENCODER_TOP_K`, default 50), return top **N** = `limit`. JSON **`score`** is **sigmoid(logit)**; **`cross_encoder_logit`** when **`CODEBASE_MCP_RERANK_DEBUG_SCORES=1`**. Batching: **`CODEBASE_MCP_CROSS_ENCODER_BATCH`**. **Not done:** timeout guard, other model families, GPU path. |
...
| **Path / language filtering** | Medium | **Partial** | **`codebase_search`** supports **`path_prefix`**, optional **`ext`** / **`lang`** / **`glob`** (see **`src/path-query-filter.ts`**, **`src/store.ts`**, **`src/mcp.ts`**). Filters are applied in the store after hybrid/vector retrieval (post-filter with over-fetch when narrowing). **`lang`** maps to known extension sets; unknown **`lang`** returns a tool error. **`glob`** uses **picomatch** on repo-relative paths. **Related:** `CODEBASE_MCP_INDEX_EXCLUDE` (daemon) skips paths from the index, not a query-time filter. |
| **`query_classifier` / `search_focus` (agent-tunable search)** | Medium–Big | **Done (initial)** | Optional args on **`codebase_search`**: **`query_classifier`** or alias **`search_focus`**: **`code`** \| **`config`** \| **`docs`** \| **`auto`**. Biases heuristic rerank (additive prior by path kind) or, when rerank is off, reorders by hybrid score + prior (`src/query-classifier.ts`, **`src/rerank.ts`**, **`src/mcp-tools.ts`**). **`docs`** bucket = markdown extensions + paths under **`CODEBASE_MCP_WORKING_DOCS_PATH`**. Conflict if both args are set to different values. Spec in **`docs/representation-inputs-plan.md`** §8. |
...
| **Definition vs usage** — boost canonical definition for “where is X defined?” | Big | **Partial (heuristic)** | **`src/tree-sitter-definitions.ts`** (supported langs) + regex merge in **`src/chunker.ts`**; **`src/definition-intent.ts`**; **`src/rerank.ts`**. **Planned upgrade:** **definition-intent hybrid** — when the query is definition-shaped (phrases *or* strong **camelCase/PascalCase** tokens), run **symbol/path lookup** against index metadata and/or **ctags**-style data, **merge** with hybrid hits (dedupe, boost defs). See **`docs/representation-inputs-plan.md`** §9. Remaining gap: very dynamic metaprogramming, some Rails/Ruby cases vs MRI, LSP-level accuracy. Reindex when chunker changes. |
...
- Cross-encoder (optional): `src/cross-encoder-rerank.ts`
- Definition intent: `src/definition-intent.ts`
- Match quality: `src/search-confidence.ts`
- Tool API: `src/mcp.ts` (`path_prefix`, `ext`, `lang`, `glob`, `limit`); path filter: `src/path-query-filter.ts`
...
2. **Cross-encoder rerank** — **Shipped (opt-in):** `CODEBASE_MCP_CROSS_ENCODER=1`, `Xenova/bge-reranker-base` by default, **`src/cross-encoder-rerank.ts`**. **Still to tune / add:** timeouts, more models, optional GPU. **Return on effort:** **High** (one integration surface, predictable quality lift on ordering; pays ops cost in latency + second model).
...
5. **API** — Optional **`lang` / `ext`** / **`glob`** on `codebase_search` (complements `path_prefix`) — **Done** in **`src/path-query-filter.ts`** + **`src/store.ts`**. **Return on effort:** **High** (narrow tool + store surface; large user-facing win for scoping generic tokens without new models).
...
7. **Query tuning + definition merge** — Ship **`query_classifier` / `search_focus`** on **`codebase_search`**; **definition-intent hybrid** (identifier + phrase triggers → AST/ctags lookup merged with hybrid results). Cross-ref **`docs/representation-inputs-plan.md`** §8–9. **Return on effort:** **High** for agent-facing control and “where defined” top-1.
8. **Confidence (next)** — Deeper than `MATCH_CONF_*`: flag wrong **single** winner when the dominant score is still in the *wrong* domain, or add BM25/lexical clash signals. **Return on effort:** **Medium** (mostly heuristics on existing lists; improves agent UX but does not fix wrong top hit by itself).
