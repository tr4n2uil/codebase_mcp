...
| **B. Ruby `Ripper` (subprocess)** | **Done (initial):** `scripts/ripper_definitions.rb` + `src/ruby-ripper.ts`; `CODEBASE_MCP_RUBY_DEF_ENGINE` (`auto` / `ripper` / `regex`); merge with regex; in-memory cache by content hash. Covers `def` / `defs`, `class` / `module` / `sclass` (singleton line), `Struct`/`Data`/`Class`/`Module` assigns, `enum` command/hash; **reindex** to apply. | **High** for syntax Ripper supports (matches MRI). | **Tradeoffs** — requires `ruby` + script in package; JRuby/TruffleRuby not targeted; `regex` if Ripper missing or per-file fail. |
